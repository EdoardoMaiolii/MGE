...
public class MeshWriterImpl implements MeshWriter {
    ...
    @Override
    public void write(final Mesh mesh, final String path) {
        final List<Point3D> points = mesh.getSegments().stream()
                .flatMap((final Segment3D seg) -> Stream.of(seg.getA(), seg.getB())).distinct()
                .collect(Collectors.toList());
        final Map<Point3D, String> pointMap = IntStream.range(0, points.size()).boxed()
                .collect(Collectors.toMap(i -> points.get(i), i -> Integer.toString(i)));
    ...
    YamlNode pointYaml(final Point3D point, final String name) {
        return Yaml.createYamlMappingBuilder().add("name", name)
                .add("x", Double.toString(point.getX())).add("y", Double.toString(point.getY()))
